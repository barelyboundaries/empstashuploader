...
def test_tier1_discovery_via_env_var(tmp_path, monkeypatch):
    """Tier 1: DEEPSEEK_BACKEND_DIR environment variable override takes top priority."""
    ...
    assert "\x01i\x02[Discovery] Resolved backend via DEEPSEEK_BACKEND_DIR:" in logs
...
def test_tier2_discovery_via_site_packages(monkeypatch):
    ...
    monkeypatch.delenv("DEEPSEEK_BACKEND_DIR", raising=False)
